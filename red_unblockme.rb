...
class Array
  def remove
    self.each{|e| e.remove if e.class.method_defined? :remove}
    clear
  end
end

class Shoes::Types::Shape
  [:vertical, :len, :x, :y].each do |m|
...
Shoes.app title: 'Unblock Me!', width: 350, height: 380 do
  extend Unblock
  background dimgray
  strokewidth 0
  ...
  b.click{cars.remove; bt.text = number[n]; run[n]}
  para link('<<'){cars.remove; bt.text = number[n=(n-1)%len]; run[n]}, left: 90, top: 10
  para link('>>'){cars.remove; bt.text = number[n=(n+1)%len]; run[n]}, left: 240, top: 10
